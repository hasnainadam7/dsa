...
void select_string(int str, int s, string **studentsList)
{
	string  temp;
	int loc;
	for(int i=0;i<4;++i)
...
void Insertion_Sort(string **studentsList, int n)
{
    int i, j;
    for (i = 0; i < n - 1; i++)
        ...
        for (j = 0; j < n - i - 1; j++)
            if (((int)toupper(studentsList[j][1][0])) > ((int)toupper(studentsList[j + 1][1][0])))
                select_string(j, j + 1, studentsList);
}
...
int main()
{
    ...
    Insertion_Sort(studentsList, num_of_students);
